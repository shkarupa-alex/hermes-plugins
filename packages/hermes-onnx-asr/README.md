...
В интерактивном setup выбор точности зависит от архитектуры. Для x86-64,
включая Ryzen, мастер предлагает `fp32`: для текущей GigaAM v3 E2E RNN-T
upstream-бенчмарк на x86-64 показывает практически одинаковую скорость
`fp32`/`int8`, поэтому `fp32` предпочтительнее при достаточной памяти. Для Apple
Silicon и прочих ARM мастер предлагает `int8`. Локальный end-to-end замер на M4
Max и минутной release-записи дал `0,326` с / 426 MiB peak RSS для `int8` против
`0,455` с / 1 037 MiB для `fp32`, с идентичной расшифровкой. Это ориентир, а не
универсальная гарантия для всех поколений CPU. `int8` занимает примерно в
четыре раза меньше места (`~0,23` против `~0,9` ГБ); оба варианта можно безопасно
сравнить на одной записи повторным запуском setup.
...
- `warmup` содержит короткий русский speech/VAD smoke; отдельный versioned
  regression corpus находится в checkout под `release-corpus/`
  (`manifest.jsonl`, `baseline.json` и указанные в manifest аудиофайлы).
  Текущий corpus намеренно состоит из одного пользовательского VK voice
  fixture с проверенной расшифровкой, лицензией, source metadata и SHA-256. Он
  не входит в wheel, но является обязательным release artifact: без него
  tag/publish gate намеренно останавливается.
