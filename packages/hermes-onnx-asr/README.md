...
включая Ryzen, и Apple Silicon мастер предлагает `fp32`: для текущей GigaAM v3
E2E RNN-T upstream-бенчмарк на x86-64 показывает практически одинаковую
скорость `fp32`/`int8`, поэтому `fp32` предпочтительнее при достаточной памяти.
`int8` занимает примерно в четыре раза меньше места (`~0,23` против `~0,9` ГБ)
и остаётся стартовым вариантом для прочих ARM/неизвестных архитектур. Реальный
выигрыш зависит от SIMD/dot-product инструкций процессора; оба варианта можно
безопасно сравнить на одной записи повторным запуском setup.
