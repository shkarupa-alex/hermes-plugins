...
Скачивание модели выполняется только после явного подтверждения. Модель занимает
...
Каталог внутри wheel фиксирует Hugging Face repository и immutable commit.
...
ONNX session. Любой CUDA, CoreML или иной provider считается фатальной ошибкой.
...
- `warmup` выполняет локальный silence smoke и CPU-аудит; отдельный русский
  эталонный corpus используется в release CI, поскольку его лицензия и хэши
